...
struct SegTree {
	...
	SegTree(int N) {
		...
		memset(mins, 0, 2 * sz * sizeof(ll));
        memset(lazy, 0, 2 * sz * sizeof(ll));
		
		for(int i = 0; i < N; i++) mins[sz + i] = arr[i]; // original values
		for(int i = sz - 1; i > 0; i--) mins[i] = min(mins[i * 2], mins[i * 2 + 1]); // calculate parent node
	}
	...
	void update(int node, int s, int e, int lo, int hi, ll x) {
		...
		mins[node] = min(mins[2 * node], mins[2 * node + 1]); // update root
	}
	...
	ll query(int node, int s, int e, int lo, int hi) {
		...
		if(s >= lo && e <= hi) return mins[node]; // completely contained
